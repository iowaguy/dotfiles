...
#+begin_src emacs-lisp
  (if (daemonp)
      (add-hook 'after-make-frame-functions
                (lambda (frame)
                  (with-selected-frame frame (hrs/apply-dark-theme))))
    (hrs/apply-dark-theme))
#+end_src
...
Images should be shown in-buffer by default.
...
#+begin_src emacs-lisp
  (setq org-agenda-files (list org-index-file org-gcal-file))
#+end_src

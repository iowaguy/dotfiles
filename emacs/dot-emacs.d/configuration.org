...
#+begin_src emacs-lisp
  ...
  (setq notmuch-saved-searches
        '((:name "inbox" :query "tag:inbox" :key "i" :sort-order newest-first)
          ...
          (:name "nu-unread" :query "to:weintraub.b@northeastern.edu AND tag:unread", :key "nu" :sort-order newest-first)
          (:name "calnewport" :query "tag:calnewport AND tag:unread" :key "cn" :sort-order newest-first)
          (:name "lightning-dev" :query "tag:lists/lightning-dev AND tag:unread" :sort-order newest-first)
          (:name "newyorker" :query "tag:newyorker AND tag:unread" :sort-order newest-first)
          (:name "scholarly-reading" :query "tag:scholarly-reading AND tag:unread" :sort-order newest-first)
          (:name "the-economist" :query "tag:economist AND tag:unread" :sort-order newest-first)
  ...
  (define-key notmuch-search-mode-map "d"
    (lambda (&optional beg end)
      "Delete me"
      (interactive (notmuch-interactive-region))
      (notmuch-search-tag (list "-unread" "-inbox" "+delete") beg end)))
